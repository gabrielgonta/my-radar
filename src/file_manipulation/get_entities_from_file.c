...
int get_entities_from_file(char const *file_path, sim_t *sim)
{
    ...
    if (!file_buffer)
        return (1);
    sim->info->nb_planes = my_count_char(file_buffer, PLANE_SYMBOL);
    sim->info->nb_towers = my_count_char(file_buffer, TOWER_SYMBOL);
    sim->planes = malloc(sizeof(plane_t *) * (sim->info->nb_planes + 1));
    sim->towers = malloc(sizeof(tower_t *) * (sim->info->nb_towers + 1));
